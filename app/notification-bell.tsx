...
import { Bell, Megaphone, ShieldAlert, X } from "lucide-react";
...
export default function NotificationBell() {
  ...
  return <div className="notification-wrap" ref={ref}>
    ...
    {open && <div className="notification-panel">
      ...
      <div className="notification-list">{items.length ? items.map(item => <div key={item.id} className={`notification-item${item.read ? " read" : ""}`} onClick={() => markRead(item)}><div className="notification-icon">{icon(item.kind)}</div><div className="notification-copy"><span>{label(item.kind)}{item.priority && item.priority !== "NORMAL" ? ` · ${item.priority}` : ""}</span><strong>{item.subject}</strong><p>{item.body}</p><small>{time(item.created_at)}</small></div><div style={{ display: "flex", alignItems: "center", gap: "8px", flex: "none" }}>{!item.read && <i className="notification-unread" />}<button className="notification-clear" aria-label={`Clear ${item.subject}`} title="Clear notification" onClick={e => { e.stopPropagation(); clearNotification(item.id); }} style={{ width: "27px", height: "27px", border: "1px solid #c3d1df", borderRadius: "5px", background: "#fff", color: "#607995", display: "grid", placeItems: "center", cursor: "pointer" }}><X size={14} /></button></div></div>) : <div className="notification-empty">No system notifications.</div>}</div>
