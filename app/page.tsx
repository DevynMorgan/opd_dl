...
function EditPersonModal({ record, onSave, onClose }: { record: RecordRow; onSave: (values: Record<string, string>) => Promise<boolean>; onClose: () => void }) {
  ...
  async function save() {
    ...
    const ok = await onSave({ first_name: first, last_name: last, dob, alias, address, eyes, hair, status, notes, license_number: licenseNumber });
    if (!ok) setError("The record could not be updated.");
    setSaving(false);
  }

  return <div className="modal-backdrop" onClick={onClose}><div className="record-modal new-record" onClick={e => e.stopPropagation()}><button className="modal-close" onClick={onClose}><X /></button><div className="modal-head"><div className="modal-icon"><Pencil /></div><div><small>ADMIN / RECORD EDIT</small><h2>EDIT PERSON RECORD</h2><p>Update the existing database record. This does not create a duplicate.</p></div></div><div className="new-form"><label>First Name<input value={first} onChange={e => setFirst(e.target.value)} autoFocus /></label><label>Last Name<input value={last} onChange={e => setLast(e.target.value)} /></label><label>Date of Birth<input type="date" value={dob} onChange={e => setDob(e.target.value)} /></label><label>Alias<input value={alias} onChange={e => setAlias(e.target.value)} placeholder="Optional" /></label><label>Address<input value={address} onChange={e => setAddress(e.target.value)} placeholder="Optional" /></label><label>License Number<input value={licenseNumber} onChange={e => setLicenseNumber(e.target.value)} placeholder="Optional OP-######" /></label><label>Eyes<input value={eyes} onChange={e => setEyes(e.target.value)} /></label><label>Hair<input value={hair} onChange={e => setHair(e.target.value)} /></label><label>Status<select value={status} onChange={e => setStatus(e.target.value)}><option>ACTIVE</option><option>INACTIVE</option><option>DECEASED</option><option>UNKNOWN</option></select></label><label className="full-width">Notes<textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} /></label></div>{error && <div className="save-alert">{error}</div>}<div className="actions modal-actions"><button className="primary" onClick={save} disabled={saving}><Pencil />{saving ? "SAVING..." : "SAVE CHANGES"}</button><button onClick={onClose} disabled={saving}>CANCEL</button></div></div></div>;
}
