...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await p.query(`
      SELECT COALESCE(MAX(CASE WHEN ${type === "incident" ? `incident_number` : `warrant_number`} ~ ${JSON.stringify(pattern)} THEN CAST(SUBSTRING(${type === "incident" ? `incident_number` : `warrant_number`} FROM LENGTH(${JSON.stringify(prefix)}) + 1) AS INTEGER) ELSE 0 END),0) AS max_number,
             COALESCE(MAX(CASE WHEN ${type === "incident" ? `incident_number` : `warrant_number`} ~ ${JSON.stringify(pattern)} THEN LENGTH(SUBSTRING(${type === "incident" ? `incident_number` : `warrant_number`} FROM LENGTH(${JSON.stringify(prefix)}) + 1)) ELSE 3 END),3) AS number_width
      FROM ${table}
    `);
